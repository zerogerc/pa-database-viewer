...
def test_merged_relations_eid1_filter(relations_db: ExtractedRelationsDatabase):
    ...
    assert len(merged) == 2
    assert merged[0] == MergedRelation(c1, g2, 'transport', prob=pytest.approx(1.0), pmids=['3'])
    assert merged[1] == MergedRelation(c1, g1, 'expression', prob=pytest.approx(0.95), pmids=['1', '2'])
